...
export class AnterosLoader extends Component {
  ...
  static removeType(type) {
    delete LoaderTypes[key]
  }

  static addType(key, nDivs) {
    return LoaderTypes[key] = nDivs;
  }
  ...
  render() {
    const nDivs = range(LoaderTypes[this.props.type]);
    ...
    const innerClasses = classnames(['loader-inner', this.props.type, this.props.innerClassName])

    return <div className={classes} style={this.props.style}>
      <div className={innerClasses}>
        {nDivs.map(v => {
          return this.renderDiv(v);
        })}
      </div>
    </div>
  }

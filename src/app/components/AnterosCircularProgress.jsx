...
export class AnterosCircularProgress extends Component {
    constructor(props) {
        super(props);
    }
    ...
    render() {
        ...
        return (
            <svg {...props} width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
                <circle cx={size / 2} cy={size / 2} r={r} style={trackStyle}/> {progress > 0
                    ? <path d={progressPath} style={progressStyle}/>
                    : null}
                {progress > 0
...
AnterosCircularProgress.propTypes = {
  ...
  AnterosCircularProgress.defaultProps = {
    startDegree: 0,
    progress: 0,
    progressWidth: 5,
    trackWidth: 5,
